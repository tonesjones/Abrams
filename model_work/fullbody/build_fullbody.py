...
from face_details import add_face_fur
...
for src in sorted(source_meshes, key=lambda o: -len(o.data.vertices)):
    ...
    for v in mesh.vertices:
        ...
        v.co = position
        # Likeness pass: keep the validated rig/body and sculpt only head-weighted
        # vertices. Both meshes use the same transform so the eyes stay seated.
        head_weight = dest_weights.get('head', 0.0)
        if head_weight:
            facial = Vector()
            for name, w in groups:
                key = short(name)
                if key.startswith('bn_lip_lower'):
                    facial.z -= .042 * w / total
                elif key.startswith('bn_teeth_lower'):
                    facial.z -= .038 * w / total
                elif key.startswith('bn_jaw'):
                    facial.z -= .025 * w / total
                elif key.startswith('bn_corner_lip'):
                    facial.z += .013 * w / total
                elif key.startswith('bn_lip_upper'):
                    facial.z += .008 * w / total
            sculpted = position + facial
            sculpted.x *= 1.18
            sculpted.z = 2.72 + (sculpted.z - 2.72) * .84
            # Round the broad muzzle while keeping the nose in front of the eyes.
            muzzle = float(np.exp(-((position.z - 2.66) / .07) ** 2))
            sculpted.x *= 1.0 + .05 * muzzle
            eye_blend = Vector()
            eye_total = 0.0
            for name, w in groups:
                key = short(name)
                if key.startswith('bn_Eye'):
                    side = 'L' if '_L' in key else 'R'
                    center = target_heads['head'] + (source_heads[by_short['bn_Eye_' + side]] - head_source) * SCALE
                    shifted = Vector((center.x * 1.18, center.y, 2.72 + (center.z - 2.72) * .84))
                    eye_blend += (shifted + (position - center) * .96) * (w / total)
                    eye_total += w / total
            sculpted = sculpted * (1.0 - eye_total) + eye_blend
            v.co = position.lerp(sculpted, head_weight)
    ...
    if obj.name == 'body':
        # Light local smoothing of the fur-bearing cheek/chin surface.
        soften = obj.vertex_groups.new(name='temporary_face_smoothing')
        for v in obj.data.vertices:
            h = weights[v.index].get('head', 0)
            if h > .8 and v.co.z < 2.89 and (abs(v.co.x) > .12 or v.co.z < 2.59):
                soften.add([v.index], .65 * h, 'REPLACE')
        bpy.context.view_layer.objects.active = obj
        smooth = obj.modifiers.new('Cheek surface cleanup', 'SMOOTH')
        smooth.factor = .6
        smooth.iterations = 3
        smooth.vertex_group = soften.name
        bpy.ops.object.modifier_apply(modifier=smooth.name)
        obj.vertex_groups.remove(obj.vertex_groups['temporary_face_smoothing'])
        add_face_fur(obj)
    obj.parent = arm
        ...
        if mat:
            ...
            if mat.use_nodes:
                ...
                if obj.name == 'body':
                    ...
                    normal_image.image = bpy.data.images.load(str(HERE.parent / 'import_head/textures/Image_1.png'), check_existing=True)
                    normal_image.image.colorspace_settings.name = 'Non-Color'
                    normal_node = nodes.new('ShaderNodeNormalMap')
                    mat.node_tree.links.new(normal_image.outputs['Color'], normal_node.inputs['Color'])
