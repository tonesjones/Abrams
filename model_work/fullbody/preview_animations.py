...
bpy.ops.wm.open_mainfile(filepath=str(HERE / 'sulley_fullbody.blend'))
...
if compiled:
    ...
    for obj in list(bpy.data.objects):
        ...
        elif len(obj.data.vertices) == 292 or (len(obj.data.vertices) > 5000 and len(obj.data.vertices) != 18942):
            kind = 'eyes' if len(obj.data.vertices) == 292 else 'body'
            obj.name = 'face' if kind == 'eyes' else 'body'
            obj.data.materials.clear()
            material = bpy.data.materials.new('Compiled preview ' + kind)
            material.use_nodes = True
            shader = material.node_tree.nodes.get('Principled BSDF')
            texture = material.node_tree.nodes.new('ShaderNodeTexImage')
            texture.image = bpy.data.images.load(str(HERE.parent / 'import_head/textures' / ('Image_2.png' if kind == 'eyes' else 'Image_0.png')), check_existing=False)
